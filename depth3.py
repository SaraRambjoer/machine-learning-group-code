...
for activationFunc in [["sigmoid", returnSigmoid], ["relu", returnRelu], ["leakyRelu", keras.layers.LeakyReLU]]:
    ...
    for num in range(0, 3):
        model01.add(keras.layers.Dense(6, activation=activationFunc[1]()))
        model001.add(keras.layers.Dense(6, activation=activationFunc[1]()))
        model0001.add(keras.layers.Dense(6, activation=activationFunc[1]()))
        model00001.add(keras.layers.Dense(6, activation=activationFunc[1]()))
        model000001.add(keras.layers.Dense(6, activation=activationFunc[1]()))
    ...
    model01.compile(loss="categorical_crossentropy", optimizer=sgd01, metrics=[
        'accuracy'])  # use crossentropy for loss, sgd optimizer (may change, must describe in report) and accuracy metric (Should also justify this in report)
    model01.fit(x, encoded_Y, verbose=0, epochs=5000, batch_size=32,
                callbacks=[tf.keras.callbacks.CSVLogger("Logs/depth3-0.1-" + activationFunc[0] + ".csv")])
    ...
    model001.fit(x, encoded_Y, verbose=0, epochs=5000, batch_size=32, callbacks=[tf.keras.callbacks.CSVLogger("Logs/depth3-0.01-" + activationFunc[0] + ".csv")])
    ...
    model0001.fit(x, encoded_Y, verbose=0, epochs=5000, batch_size=32, callbacks=[tf.keras.callbacks.CSVLogger("Logs/depth3-0.001-" + activationFunc[0] + ".csv")])
    ...
    model00001.fit(x, encoded_Y, verbose=0, epochs=5000, batch_size=32, callbacks=[tf.keras.callbacks.CSVLogger("Logs/depth3-0.0001-" + activationFunc[0] + ".csv")])
    ...
    model000001.fit(x, encoded_Y, verbose=0, epochs=5000, batch_size=32, callbacks=[tf.keras.callbacks.CSVLogger("Logs/depth3-0.00001-" + activationFunc[0] + ".csv")])
    model01.save("models/depth3-" + activationFunc[0])
    model001.save("models/depth3-" + activationFunc[0])
    model0001.save("models/depth3-" + activationFunc[0])
    model00001.save("models/depth3-" + activationFunc[0])
    model000001.save("models/depth3-" + activationFunc[0])
